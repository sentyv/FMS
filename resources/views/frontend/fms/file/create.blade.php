...
@section('title')
    @lang('buttons.general.crud.create') File
@endsection
...
@section('content')
{{ html()->form('P0ST',route('frontend.fms.file.store'))->attribute('enctype',"multipart/form-data" )->open()}}

    <!-- <div class="form-group">
        <label class="" for="referenceName">File Reference Code<i id="loading_reference" class="fas fa-spinner fa-spin"></i></label>
        {{ html()->input('hidden')->id('file_reference_id')->name('file_reference_id')->value(old('file_reference_id')) }}
        {{ html()->input('search')->class('form-control')->required()
                           ->name('file_reference_name')->id('file_reference_name')->attribute('autocomplete','off')
                           ->value(old('file_reference_name'))}}
        <small id="reference-notes" class="form-text text-muted"></small>
    </div> -->

    <div class="form-group">
    {{ html()->input('hidden')->id('file_reference_id')->name('file_reference_id')->value(old('file_reference_id')) }}
        {{ html()->label('File Index')->class('form-control-label')->for('file_reference_id') }}
        <select class="form-control" name="file_reference_id" id="file_reference_id">
            @foreach($references as $key => $name)
                <option value="{{ $key }}" {{ ($key == old('file_reference_id')) ? 'checked' : '' }}>{{ $name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <input type="text" class="form-control" id="file_subject_title" name="file_subject_title" placeholder ="Enter Subject Title" value="{{ old('file_subject_title') }}">
    </div>

    <div class="form-group">
    <label for="file_author_name">File Author Name</label>
    <input type="text" class="form-control" id="file_author_name" name="file_author_name" placeholder ="Enter Author Title" value="{{ old('file_author_name') }}">
    </div>

    <div class="form-group">
    <label for="file_published_date">File Published Date</label>
    <input type="date" class="form-control" id="file_published_date" name="file_published_date" value="{{ old('file_published_date') }}">
    </div>


    <div class="form-group">
    <label for="file_recieved_date">File Received Date</label>
    <input type="date" class="form-control" id="file_received_date" name="file_received_date" value="{{ old('file_received_date') }}">
    </div>
       ...
       <div class="form-group">
        ...
        <select class="form-control" name="file_publisher_id" id="file_publisher_id">
            @foreach($publishers as $key => $name)
                <option value="{{ $key }}" {{ ($key == old('file_publisher_id')) ? 'checked' : '' }}>{{ $name }}</option>
            @endforeach
        </select>
    </div>
    ...
    <!-- <div class="form-group">
        ...
        <div class="form-check">
            <input class="form-check-input" type="checkbox" value="true" id="status" name="status" {{ (old('status', false)) ? 'checked' : ''}}>
    ...
    <!-- <div class="form-group">
    <label for="file_image">File Upload</label>
